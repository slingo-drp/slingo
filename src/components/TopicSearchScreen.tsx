...
type TopicSearchScreenProps = {
  ...
  onToggleTopic: (topic: Domain) => void;
  onResetFilters: () => void;
};
  ...
  return (
    <View className="flex-1 bg-app-background">
      ...
      >
        ...
        <Card variant="app">
          <Text variant="sectionLabel">{topicsLabel}</Text>
          ...
          <View className="flex-row flex-wrap gap-2">
            {LESSON_TOPICS.map((topic) => {
              ...
              return (
                <SelectableChip
                  key={topic}
                  accessibilityLabel={`${title} by ${formatTopicLabel(topic)}`}
                  accessibilityRole="button"
                  active={active}
